...
class KMeansSuite extends FunSuite {
  ...
  test("kmeans test with points, means and eta") {
    val p1 = new Point(0.0, 0.0, 1.0)
    val p2 = new Point(0.0, 0.0, -1.0)
    val p3 = new Point(0.0, 1.0, 0.0)
    val p4 = new Point(0.0, 10.0, 0.0)
    val points: GenSeq[Point] = Vector(p1, p2, p3, p4)
    val mean1 = new Point(0.0, -1.0, 0.0)
    val mean2 = new Point(0.0, 2.0, 0.0)
    val means: GenSeq[Point] = Vector(mean1, mean2)
    val eta = 12.25
    val kMeans = new KMeans()
    val result: GenSeq[Point] = kMeans.kMeans(points, means, eta)
    val resMean1 = new Point(0.0, 0.0, 0.0)
    val resMean2 = new Point(0.0, 5.5, 0.0)
    val expected: GenSeq[Point] = Vector(resMean1, resMean2)
    assert(result == expected, s"$expected should equal $result")
  }
