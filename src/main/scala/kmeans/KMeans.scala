...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    for {
      (k, v) <- meansMap
      value = if (map contains k) map(k) else v
    } yield k -> value
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    for (oldMean <- oldMeans) yield findAverage(oldMean, classified(oldMean))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    while (i < oldMeans.length) {
      if (oldMeans(i).squareDistance(newMeans(i)) <= eta) i += 1
      else {
        i = oldMeans.length
        bool = false
      }
...
class Point(val x: Double, val y: Double, val z: Double) {
  ...
  override def equals(that: scala.Any): Boolean =
    if (!that.isInstanceOf[Point]) false
    else {
      that match {
